...
from src.data.protein import Pocket, AtomRepr
...
class PocketStructureStreamer(GeneratorStreamer):
    ...
    def put_generator(self):
        ...
        if self.pocket_h in ['all', 'atom']:
            n_atom = len(self.pocket.atoms)
        else:
            n_atom = np.sum(slice_str(self.pocket.atoms, 1) != 'H')
        start_position = 2 if self.format == 'ordered_atoms_coords' else len(prompt_tokens)
        coords, pos, error = yield from coord_streamer(n_atom, start_position, self.new_coord_path, self.voc_encoder, self.coord_range, self.no_token_range, self.format == 'ordered_atoms_coords')
...
if __name__ == '__main__':
    ...
    pocket_token = PocketTokenizeDataset(pocket, heavy=args.pocket_heavy, h=args.pocket_h, format=targs.pocket_format, coord_range=targs.coord_range)
    ...
    def streamer_fn(item, i_trial: int, voc_encoder: VocEncoder):
        ...
        streamer = PocketStructureStreamer(
            ...
            pocket=item[1],
            coord_range=targs.coord_range, no_token_range=args.no_token_range, format=targs.format, pocket_h=targs.pocket_h
        )
