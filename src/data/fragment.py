...
def _process3_init(radius_mean_i, radius_std_i, max_n_atom_i):
    global parser, ppbuilder, amino_resnames, radius_mean, radius_std, max_n_atom, logger
    parser = FastMMCIFParser(QUIET=True)
    ppbuilder = PPBuilder()
    amino_resnames = {
    ...
    try:
        ...
        while not np.all(sample_finished):
            ...
            data = {
                'name': name,
                'sub_idx': sub_idx,
                'atoms': names[sub_idxs],
                'coordinate': coords[sub_idxs],
            }
            datas.append(data)
            sub_idx+=1
        return datas
...
def process_fragment3(root_dir: str, out_dir: str,
        radius_mean: float, radius_std: float,
        reset=False, num_workers: int=1, tqdm: bool=False, max_tasks_per_child: int=None,
        range_min: int=None, range_sup: int=None, max_n_atom: int=None):
    """
    PDBFragmentDatasetと同じだが, データをポケットごとに保存する。
    ...
    with cf.ProcessPoolExecutor(num_workers, initializer=_process3_init, 
            initargs=(radius_mean, radius_std, max_n_atom), max_tasks_per_child=max_tasks_per_child) as e:
